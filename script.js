...
function printPattern(){
  console.log(pattern) ;
  let nonZeroValues = pattern.map((value, index) => ({ value, index: index + 1 })).filter(item => item.value !== 0);
  // Sort the non-zero values in ascending order
  nonZeroValues.sort((a, b) => a.value - b.value);

  // Extract the indices of the sorted values
  let sortedIndices = nonZeroValues.map(item => item.index);
  globalArray = sortedIndices;
  console.log(sortedIndices);
$(outputId).html("Pattern="+sortedIndices);
 let array2=[7,5,8,6];
if (JSON.stringify(sortedIndices) === JSON.stringify(array2)) {
  window.open('https://mahanthivikas.com','_blank');
}
}
...
// ...
lockPatternContainer.addEventListener('touchstart', (e) => {
  e.preventDefault();
  lockPatternContainer.classList.add('drawing');
  pattern.length = 0;
});

lockPatternContainer.addEventListener('touchmove', (e) => {
  e.preventDefault();
  if (lockPatternContainer.classList.contains('drawing')) {
      const point = document.elementFromPoint(e.touches[0].clientX, e.touches[0].clientY);
      if (point && point.classList.contains('point')) {
          if (!pattern.includes(point.dataset.index)) {
              pattern.push(point.dataset.index);
...
lockPatternContainer.addEventListener('touchend', () => {
  lockPatternContainer.classList.remove('drawing');
  checkPattern();
});
// ...
